...
class LLM():
    ...
    def remove_think_tags(self,
                          response: str):
        """
        For DeepSeek-R1, only keep the content after the last </think>.

        Keyword arguments:
        response -- string generated by an LLM
        """
        last_think = response.rfind("</think>")
        if last_think == -1:
            return response
        response_without_think = response[last_think+8:]
