...
import json
import requests
from config import OLLAMA_HOST, LLM_CATEGORIES_FILE, OLLAMA_MODEL, OLLAMA_TEMPERATURE  # type: ignore
...
class LLM():
    ...
    def __new__(cls, *args, **kwds):
        ...
        if not hasattr(cls, '_instance'):
            ...
            cls._load_llm_results_from_cache(cls)
    ...
    def _save_llm_results_in_cache(self):
        if self.llm_categories:
            ...
            print(f"dumping {len(self.llm_categories)} LLM results in {str(LLM_CATEGORIES_FILE)}.")
            with open(LLM_CATEGORIES_FILE, "w", encoding = "utf-8") as f:
                json.dump(self.llm_categories, f, indent=" ")


    def _load_llm_results_from_cache(self):
        atexit.register(self._save_llm_results_in_cache, self)
