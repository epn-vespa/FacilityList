...
# Fonction pour transformer le titre en identifiant
def title_to_id(title):
    return re.sub(r'\s+', '-', title.strip().lower())

# Fonction pour enlever les caractères non-ASCII
def remove_non_ascii(text):
    return re.sub(r'[^\x00-\x7F]+', '', text)

# Fonction pour nettoyer la liste skos:altLabel
def clean_alt_labels(alt_labels):
    return [remove_non_ascii(label) for label in alt_labels]

# Fonction pour comparer et extraire les données
...
output_path = '/Users/ldebisschop/PycharmProjects/FacilityList/data/obs-facilities_vocabulary/obsfacility-vocabulary_v_1.2.json'
with open(output_path, 'w') as f:
    json.dump(merged_data, f, ensure_ascii=False, indent=4)

print("Merging completed. Results saved to obsfacility-vocabulary_v_1.2.json")
