...
altname_str = consts.ALTERNATE_NAME
name_str = consts.NAME
id_str = consts.ID
auth_str = consts.NAMING_AUTHORITY
targetlist_str = consts.TARGET_LIST
facilitytype_str = consts.FACILITY_TYPE
facilitygroup_str = consts.FACILITY_GROUP
location_str = consts.LOCATION
continent_str = consts.CONTINENT
country_str = consts.COUNTRY
coordinates_str = consts.COORDINATES
measurementtype_str = consts.MEASUREMENT_TYPE
referenceurl_str = consts.REFERENCE_URL
launchdate_str = consts.LAUNCH_DATE
lon_str = consts.LON
lat_str = consts.LAT
alt_str = consts.ALT
tz_str = consts.TZ
...
update = False
for arg in sys.argv:
    if arg == "-f" or arg == "-fuzzy":
        fuzzy = True
    if arg == "-i" or arg == "-interactive":
        interactive = True
    if arg == "-p" or arg == "-partial":
        partial = True
        ...
        for entry in existing_list:
            if entry in new_list:
                if existing_list[entry] == new_list[entry]:
                    ...
                    while True:
                        print "\nWARNING: object in updated list from authority '" + authority + \
                              "' shares same key (" + entry + ") as in exsiting list but differs regading content: \n"
                        print "<<<<<<< Existing Object:"
                        print json.dumps( existing_list[entry], indent=2 )
                        print ">>>>>>> New Object:"
                        print json.dumps( new_list[entry], indent=2 )
                        cmd = raw_input("Type 'o' to keep the OLD object in existing list or 'n' to overwrite with "
                                        "NEW object in updated list:")
                        ...
                        if cmd.upper() == "O":
                            ...
                            print "Input not valid! Type 'o' to keep the OLD object in existing list or 'n' to " \
                                  "overwrite with NEW object in updated list:"
...
def merge_entries(doubles_array, list, logfile):
    ...
    if logfile is not None:
        write(logfile, "-> Merged entries: " + double_strings[:-2])
...
def merge_doubles(list):
    ...
    if not fuzzy and not partial:
        # no flags provided, simpole merge of identical names/ids
        ...
        for rec in list:
            ...
            if list[rec].has_key('location') \
                    and list[rec]['location'].has_key('coordinates') \
                    and list[rec]['location']['coordinates'].has_key('lon') \
                    and list[rec]['location']['coordinates'].has_key('lat'):
                lon_lat_str = trunc(list[rec]['location']['coordinates']['lon']) + ", " + \
                              trunc(list[rec]['location']['coordinates']['lat'])
        ...
        while True:
                ...
                if list[rec].has_key(altname_str):
                    for altname in list[rec][altname_str]:
                        if altname.has_key(name_str):
                            name_index[altname[name_str]] = {
                                'auth':altname[auth_str],
                                'obj':rec,
                                'cln': re.sub ('[/\-*+#,\s\.\'\"]', '', altname[name_str]).upper()
            ...
            for checked_key in name_index.keys():
                ...
                if not list.has_key(name_index[checked_key]['obj']):
                    continue

                for key in name_index.keys():
                    if not list.has_key(name_index[checked_key]['obj']):
                        break
                    if not list.has_key(name_index[key]['obj']):
                        continue
                    ...
                    if (name_index[checked_key]['cln'] in name_index[key]['cln']) and \
                            (name_index[key]['auth'] != name_index[checked_key]['auth'] and
                                 (name_index[checked_key]['obj'] != name_index[key]['obj'])):

                        found_fuzzy = True

                        hints_str = "Possibly related objects:\n\n-> " + name_index[key]['obj'] + " - '" + key + \
                                    "' (" + name_index[key]['auth'] + ")\n\n-> " + name_index[checked_key]['obj'] + \
                                    " - '" + checked_key + "' (" + name_index[checked_key]['auth'] + ")\n"
                        hints_str = hints_str
                        ...
                        if not interactive:
                            ...
                            merged_entry = merge_entries([name_index[checked_key]['obj'], name_index[key]['obj']],
                                                         list, None)
                            print "\n"

                            while True:
                                cmd = raw_input ("Type 'm' to merge, 'i' to ignore, 'd' to defer decision, "
                                                 "'s' to save merged file and resume later or '?' followed by the "
                                                 "object id for information ('*' can be used at the end of object "
                                                 "name for matching): " )
                                ...
                                if cmd.startswith("?"):
                                    cmd_arr = cmd.split()
                                    if len(cmd_arr) != 2:
                                        print "\n'?' must be followed by at least on blank and a valid object id " \
                                              "(as listed above), e.g. '? xyz:abc'!\n"
                                    else:
                                        found = False
                                        for obj_id in list:
                                            o_name = str(cmd_arr[1])
                                            if (o_name.endswith("*") and str(obj_id).startswith(o_name[:-1])) or \
                                                    (o_name.endswith("*") and str(obj_id) == cmd_arr[1][:-1]) or \
                                                    (str(obj_id) == o_name):
                                                print ("\n** Object: " + obj_id + " **")
                            ...
                            if cmd == "d":
                                # defer
                                print("\nDecision is deferred - possibly related objects will be presented again in "
                                      "the next iteration.")
                                print("\n----------------------------------------------------------------------------"
                                      "\n")
                                continue
        ...
        if not interactive:
            print(str(num_hints) + " hints created regarding objects that could be related in file '" +
                  fuzzy_hints_file_name + "'")
            if num_ignored:
                print(str(num_ignored) + " entries skipped since they were previously ignored in interactive mode.")
        else:
            save_checked_file(merged_counter)
    ...
    # "partial" search for similar keys in id/names...
        ...
        while True:
                    ...
                    for altname in list[rec][altname_str]:
                        if altname.has_key(name_str):
                            ...
                            for word in words:
                                ...
                                if len(upper_word) > 2 \
                                        and not stopwords.has_key(upper_word) \
                                        and not _checked_.has_key("partial_check::" + upper_word):

                                    if partial_index.has_key(upper_word):

                                        if partial_index[upper_word].has_key(rec):
                                            partial_index[upper_word][rec].append([altname[name_str],
                                                                                   altname[auth_str]])
                                        else:
                                            partial_index[upper_word][rec] = [[altname[name_str],
                                                                               altname[auth_str]]]
            ...
            if not found_matches:
                break # no more double matches in partial search for names were found, exit loop
            ...
            for word in partial_index:
                ...
                if not(partial_index[word]["mult"]):
                    continue
                    # only one object contains the word

                hints_str = "The term '" + word + "' appears in at least one of the alternate names of the following " \
                                                  "objects:\n"
                ...
                if not interactive:
                    ...
                    while True:
                        try:
                            cmd = raw_input("Please provide the ids of objects to be merged separated by blanks "
                                            "(e.g. '1 2'), 'a' to merge all objects, 'i' to ignore matches for term "
                                            "'{}', 'd' to defer decision or 's' to save and continue later. "
                                            "Use '?' and object number for detailed information: ".format(word))
