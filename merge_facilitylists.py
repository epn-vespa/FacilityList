	...
	for entry in entry_array:
		...
		if entry.has_key(altname_str):
			...
			for name in entry[altname_str]:
				if ( not( name.has_key(id_str) and merged_ids.has_key(name[id_str]) ) and not( name.has_key(name_str) and merged_names.has_key(name[name_str]) ) ):
					merged_entry[altname_str].append ( name )
					if name.has_key(name_str): merged_names[name[name_str]] = True;
					if name.has_key(id_str): merged_ids[name[id_str]] = True;
				...
				if list[rec].has_key(altname_str):
					for altname in list[rec][altname_str]:
						if altname.has_key(name_str):
							name_index[altname[name_str]] = { 'auth':altname[auth_str], 'obj':rec, 'cln': re.sub ('[/\-*+#,\s\.\'\"]', '', altname[name_str]) }
		...
		while True:
				...
				if list[rec].has_key(altname_str):
					for altname in list[rec][altname_str]:
						if altname.has_key(name_str):
							...
							for word in words:
								if len(word) > 2 and not( stopwords.has_key(word.upper()) ) and not( _checked_.has_key("partial_check::" + word ) ):
									if partial_index.has_key(word):
										
										if partial_index[word].has_key(rec):
											partial_index[word][rec].append( [altname[name_str], altname[auth_str]] )
										else:	
											partial_index[word][rec] = [ [altname[name_str], altname[auth_str]] ]
											partial_index[word]["mult"] = True
											found_matches = True
										
									else:
										partial_index[word] = {}
										partial_index[word][rec] = [ [altname[name_str], altname[auth_str]] ]
										partial_index[word]["mult"] = False
			...
			for word in partial_index:
				...
				if not(interactive): 
					...
					if cmd == "i": # ignore
						_checked_[ "partial_check::" + word ] = True
